...
public class UserClient {
    ...
    public List<User> getUsers(List<String> userIds, String correlationId) {
        ...
        var response = restTemplate.exchange(url, HttpMethod.GET, createEntityWithHeaders(correlationId), User[].class);
        return response.getBody() != null ? List.of(response.getBody()) : List.of();
    }

    public List<User> getUsersByEmail(List<String> emails, String correlationId) {
        String url = UriComponentsBuilder.fromUriString(userServiceUrl)
            .path("/usersgroups-query-api/query/api/rest/usersgroups/users")
